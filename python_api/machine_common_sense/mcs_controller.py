...
class McsController(Controller):
    """
    ...
    def validate_and_convert_params(self, action, **kwargs):
        ...
        if not Util.is_number(objectDirectionX, self.OBJECT_DIRECTION_X_KEY):
            objectDirectionX = self.DEFAULT_DIRECTION

        if not Util.is_number(objectDirectionY, self.OBJECT_DIRECTION_Y_KEY):
            objectDirectionY = self.DEFAULT_DIRECTION

        if not Util.is_number(objectDirectionZ, self.OBJECT_DIRECTION_Z_KEY):
            objectDirectionZ = self.DEFAULT_DIRECTION

        # Check receptacle directions are numbers
        if not Util.is_number(receptacleObjectDirectionX, self.RECEPTACLE_DIRECTION_X):
            receptacleObjectDirectionX = self.DEFAULT_DIRECTION

        if not Util.is_number(receptacleObjectDirectionY, self.RECEPTACLE_DIRECTION_Y):
            receptacleObjectDirectionY = self.DEFAULT_DIRECTION

        if not Util.is_number(receptacleObjectDirectionZ, self.RECEPTACLE_DIRECTION_Z):
            receptacleObjectDirectionZ = self.DEFAULT_DIRECTION

        # Check that params that should fall in a range are in that range
        horizon = Util.is_in_range(horizon, self.MIN_HORIZON, self.MAX_HORIZON, self.DEFAULT_HORIZON, \
                self.HORIZON_KEY)
        amount = Util.is_in_range(amount, self.MIN_AMOUNT, self.MAX_AMOUNT, self.DEFAULT_AMOUNT, self.AMOUNT_KEY)
        force = Util.is_in_range(force, self.MIN_FORCE, self.MAX_FORCE, self.DEFAULT_FORCE, self.FORCE_KEY)
    ...
    def step(self, action, **kwargs):
        ...
        history_item = SceneHistory(step=self.__step_number, action=action, args=kwargs, params=params, \
                output=output_copy)
        self.__history_list.append(history_item)
    ...
    def filter_history_images(self, history: SceneHistory) -> SceneHistory:
    ...
    def retrieve_goal(self, scene_configuration):
        ...
        return self.restrict_goal_output_metadata(Goal(
    ...
    def wrap_output(self, scene_event):
        ...
        step_output = self.restrict_step_output_metadata(StepOutput(
