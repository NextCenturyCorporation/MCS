...
class Util:
    ...
    @staticmethod
    def class_to_str(input_class, depth=0) -> str:
        ...
        for prop_key, prop_value in props.items():
            text_list.append(next_indent + "\"" + prop_key + "\": " + Util.value_to_str(prop_value, depth + 1))
    ...
    def generate_pretty_object_output(object_list: List) -> List[str]:
        ...
        rows = [titles] + [[metadata.uuid, metadata.shape, ", ".join(metadata.texture_color_list), metadata.held, \
                Util.vector_to_string(metadata.position), Util.vector_to_string(metadata.dimensions), \
                metadata.distance_in_world, Util.vector_to_string(metadata.direction)] for metadata in object_list]
    ...
    @staticmethod
    def input_to_action_and_params(input_str: str) -> Tuple[str, Dict]:
    ...
    @staticmethod
    def is_in_range(value: float, min_value: float, max_value: float, default_value: float, label: str=None) -> float:
    ...
    @staticmethod
    def value_to_str(input_value: Any, depth: int=0) -> str:
        ...
        if isinstance(input_value, dict):
            ...
            for dict_key, dict_value in input_value.items():
                text_list.append(next_indent + "\"" + dict_key + "\": " + Util.value_to_str(dict_value, depth + 1))
            return "{}" if len(text_list) == 0 else "{\n" + (",\n").join(text_list) + "\n" + this_indent + "}"
        if isinstance(input_value, list) or isinstance(input_value, tuple):
            text_list = []
            for list_item in list(input_value):
                text_list.append(next_indent + Util.value_to_str(list_item, depth + 1))
            return "[]" if len(text_list) == 0 else "[\n" + (",\n").join(text_list) + "\n" + this_indent + "]"
    ...
    @staticmethod
    def vector_to_string(vector: Dict) -> str:
    ...
    @staticmethod
    def verify_material_enum_string(enum_string: str) -> bool:
        ...
        """        
        try:
            enum_instance = Material[enum_string.upper()]
            return True
        except KeyError:
            return False
