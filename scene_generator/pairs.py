...
class SceneOptions():
    def __init__(self,
                 ...
                 obstructor_definition: Dict[str,
                                             Any] = None):
...
class InteractionPair():
    ...
    def _get_both_goal_type_list(self,
                                 options: SceneOptions,
                                 prefix: str,
                                 is_true_func: Callable[[BoolPairOption],
                                                        bool]) -> List[str]:
    ...
    def _initialize_each_goal(self) -> None:
        ...
        if self._options.confusor != BoolPairOption.NO_NO:
            for _ in range(util.MAX_TRIES):
                ...
                if must_containerize_confusor:
                    confusor_receptacle_definition_list = containers.find_suitable_enclosable_list(
                        confusor_template, target_receptacle_definition_list if must_containerize_target else None)
        ...
        if must_containerize_target or must_containerize_confusor:
            must_hold_both = (
                self._is_true_goal_1(
                    self._options.confusor) and self._is_true_goal_1(
                    self._options.target_containerize) and self._is_true_goal_1(
                    self._options.confusor_containerize) and is_confusor_close_in_goal_1) or (
                self._is_true_goal_2(
                    self._options.confusor) and self._is_true_goal_2(
                    self._options.target_containerize) and self._is_true_goal_2(
                    self._options.confusor_containerize) and is_confusor_close_in_goal_2)
            ...
            # individually.
                ...
                receptacle_data = self._create_receptacle_around_either(
                    target_definition_if_containerize, confusor_definition_if_containerize)
                if not receptacle_data:
                    raise exceptions.SceneException(
        ...
        larger_of_target_or_obstructor = self._find_larger_object(
            larger_of_target_or_receptacle, obstructor_template)
        ...
        is_confusor_relative_to_performer_start = (
            confusor_template and self._options.confusor_location == ConfusorLocationPairOption.BACK_FRONT)
        ...
        if is_target_relative_to_performer_start:
            location_front, location_back = self._generate_front_and_back(
                self._goal_1, larger_object, goal_1_target_rule_list[target_choice], True)
            ...
            if is_confusor_relative_to_performer_start:
                location_front, location_back = self._generate_front_and_back(
                    self._goal_1, larger_of_confusor_or_receptacle, goal_1_target_rule_list[target_choice], True)
                confusor_location_1 = location_back
                confusor_location_2 = location_front

            # If the target isn't positioned relative to the performer_start,
            # it'll be positioned randomly.
            target_location_1, target_location_2 = self._generate_random_location(
                self._goal_1, larger_of_target_or_receptacle, goal_1_target_rule_list[target_choice], goal_1_target_list, shared_bounds_list)
    ...
    def _create_receptacle_around_both(self,
                                       target_instance: Dict[str,
                                                             Any],
                                       confusor_instance: Dict[str,
                                                               Any]) -> Optional[Tuple[Dict[str,
                                                                                            Any],
                                                                                       int,
                                                                                       float,
                                                                                       float,
    ...
    def _create_receptacle_around_either(self,
                                         target_instance: Dict[str,
                                                               Any],
                                         confusor_instance: Dict[str,
                                                                 Any]) -> Optional[Tuple[Dict[str,
    ...
    def _finalize_goal_1(self,
                         target_1: Dict[str,
                                        Any],
                         confusor_list: List[Dict[str,
                                                  Any]],
                         distractor_list: List[Dict[str,
                                                    Any]],
                         obstructor_list: List[Dict[str,
                                                    Any]]) -> None:
    ...
    def _finalize_goal_2(self,
                         target_choice: int,
                         confusor_list_1: List[Dict[str,
                                                    Any]],
                         distractor_list_1: List[Dict[str,
                                                      Any]],
                         obstructor_list_1: List[Dict[str,
                                                      Any]],
                         target_2: Dict[str,
                                        Any],
                         confusor_list_2: List[Dict[str,
                                                    Any]],
                         distractor_list_2: List[Dict[str,
                                                      Any]],
                         obstructor_list_2: List[Dict[str,
                                                      Any]]) -> None:
        """Finalize the target and other objects in the first goal."""
    ...
    def _finalize_obstructor(self,
                             show_obstructor: bool,
                             obstructor_definition: Dict[str,
                                                         Any],
                             obstructor_template: Dict[str,
                                                       Any],
                             obstructor_location: Dict[str,
                                                       float],
    ...
    def _finalize_target_confusor_position(self,
                                           target_definition: Dict[str,
                                                                   Any],
                                           target_template: Dict[str,
                                                                 Any],
                                           ...
                                           confusor_definition: Dict[str,
                                                                     Any],
                                           confusor_template: Dict[str,
                                                                   Any],
                                           confusor_location: Dict[str,
                                                                   Any],
                                           containerize_confusor: bool,
                                           receptacle_definition: Dict[str,
                                                                       Any],
                                           receptacle_template: Dict[str,
                                                                     Any],
                                           area_index: int,
                                           ...
                                           bounds_list: List[List[Dict[str,
                                                                       float]]],
                                           confusor_receptacle_id: int) -> Tuple[Dict[str,
                                                                                      Any],
                                                                                 Dict[str,
                                                                                      Any],
                                                                                 Dict[str,
    ...
    def _generate_confusor_location(self,
                                    confusor_definition: Dict[str,
                                                              Any],
                                    target_definition: Dict[str,
                                                            Any],
                                    target_template: Dict[str,
                                                          Any],
                                    target_location_1: Dict[str,
                                                            float],
                                    target_location_2: Dict[str,
                                                            float],
                                    performer_start: Dict[str,
                                                          Dict[str,
                                                               float]],
                                    ...
                                    is_target_location_same_in_both: bool) -> Tuple[Dict[str,
                                                                                         float],
                                                                                    Dict[str,
                                                                                         float]]:
        ...
        if is_confusor_close_in_goal_1 and is_confusor_close_in_goal_2 and is_target_location_same_in_both:
            confusor_location = self._generate_location_close_to_object(
                confusor_definition, target_definition, target_location_1, performer_start)
        ...
        if is_confusor_close_in_goal_2:
            confusor_location_2 = self._generate_location_close_to_object(
                confusor_definition, target_definition, target_location_2, performer_start)
        else:
    ...
    def _generate_location_close_to_object(self,
                                           object_definition: Dict[str,
                                                                   Any],
                                           target_definition: Dict[str,
                                                                   Any],
        ...
        location_close = geometry.get_adjacent_location(
            object_definition, target_definition, target_location, performer_start)
    ...
    def _generate_location_far_from_object(self,
                                           object_definition: Dict[str,
                                                                   Any],
                                           target_location: Dict[str,
                                                                 float],
    ...
    def _generate_obstructor_location(self,
                                      show_obstructor: bool,
                                      target_definition: Dict[str,
                                                              Any],
                                      target_location: Dict[str,
        ...
        if show_obstructor and obstructor_definition:
            ...
            obstructor_location = geometry.get_adjacent_location(
                obstructor_definition, target_definition, target_location, performer_start, True)
...
class Pair1(InteractionPair):
    ...
    def __init__(self,
        ...
        containerize = BoolPairOption.YES_YES if random.random(
        ) < InteractionGoal.OBJECT_RECEPTACLE_CHANCE else BoolPairOption.NO_NO
        super(
            Pair1,
            self).__init__(
            1,
