...
def verify_body(body, goal):
    assert body['passive']
    assert body['answer']['choice'] == 'plausible'

    last_step = (60 if goal.is_move_across() else 40)
...
def verify_fall_down_object(instance, name):
    ...
    if instance['shows'][0]['position']['z'] not in z_position_list:
        print(f'[ERROR] {name} Z POSITION SHOULD BE FROM {z_position_list}\n'
              f'{name}={instance}')
        return False
    if instance['shows'][0]['position']['y'] != IntPhysGoal.FALL_DOWN_OBJECT_Y:
...
def verify_fall_down_object_list(target_list, distractor_list):
    for target in target_list:
        assert verify_fall_down_object(target, 'TARGET')
    for distractor in distractor_list:
        assert verify_fall_down_object(distractor, 'DISTRACTOR')
...
def verify_fall_down_occluder_list(occluder_list, target_list):
...
def verify_move_across_object(instance, name):
    # Verify object X and Z position.
    if (
        abs(instance['shows'][0]['position']['x']) == 4.2 or
        abs(instance['shows'][0]['position']['x']) == 5.3
    ):
        if instance['shows'][0]['position']['z'] != IntPhysGoal.OBJECT_NEAR_Z:
            print(f'[ERROR] {name} Z POSITION SHOULD BE '
                  f'{IntPhysGoal.OBJECT_NEAR_Z}\n{name}={instance}')
            return False
    elif (
        abs(instance['shows'][0]['position']['x']) == 4.8 or
        abs(instance['shows'][0]['position']['x']) == 5.9
    ):
        if instance['shows'][0]['position']['z'] != IntPhysGoal.OBJECT_FAR_Z:
            print(f'[ERROR] {name} Z POSITION SHOULD BE '
                  f'{IntPhysGoal.OBJECT_FAR_Z}\n{name}={instance}')
            return False
    else:
        x_position_list = [-5.9, -5.3, -4.8, -4.2, 4.2, 4.8, 5.3, 5.9]
        print(f'[ERROR] {name} X POSITION SHOULD BE FROM {x_position_list}\n'
              f'{name}={instance}')
        return False
    ...
    for i in range(len(instance['intphysOption']['positionByStep']) - 1):
        ...
        next_position = instance['intphysOption']['positionByStep'][i + 1]
        if left_to_right:
            if position >= next_position:
                print(f'[ERROR] LEFT-TO-RIGHT {name} POSITION BY STEP SHOULD '
                      f'BE INCREASING\n{name}={instance}\nPOSITION={position}'
                      f'\nNEXT_POSITION={next_position}')
                return False
        else:
            if position <= next_position:
                print(f'[ERROR] RIGHT-TO-LEFT {name} POSITION BY STEP SHOULD '
                      f'BE DECREASING\n{name}={instance}\nPOSITION={position}'
                      f'\nNEXT_POSITION={next_position}')
                return False
    ...
    # Verify object force properties.
    if instance['forces'][0]['stepBegin'] != instance['shows'][0]['stepBegin']:
        print(f'[ERROR] {name} FORCE STEP BEGIN SHOULD BE SAME AS SHOW STEP '
              f'BEGIN\n{name}={instance}')
        return False
...
def verify_move_across_object_list(target_list, distractor_list):
    for target in target_list:
        assert verify_move_across_object(target, 'TARGET')
    for distractor in distractor_list:
        assert verify_move_across_object(distractor, 'DISTRACTOR')
...
def verify_move_across_occluder_list(occluder_list, target_list):
...
def test_ObjectPermanenceGoal_compute_objects():
    ...
    assert verify_goal(tag_to_objects, wall_material_tuple[0])
    assert len(tag_to_objects['target']) == 1

    if goal.is_move_across():
        assert 0 <= len(tag_to_objects['distractor']) <= 2
        assert verify_move_across_object_list(tag_to_objects['target'],
                                              tag_to_objects['distractor'])
        assert 2 <= len(tag_to_objects['occluder']) <= 8
        assert verify_move_across_occluder_list(tag_to_objects['occluder'],
                                                tag_to_objects['target'])
        assert goal._last_step == 60

    if goal.is_fall_down():
        assert 0 <= len(tag_to_objects['distractor']) <= 1
        assert verify_fall_down_object_list(tag_to_objects['target'],
                                            tag_to_objects['distractor'])
        assert 2 <= len(tag_to_objects['occluder']) <= 4
        if len(tag_to_objects['distractor']) == 1:
            assert len(tag_to_objects['occluder']) == 4
        assert verify_fall_down_occluder_list(tag_to_objects['occluder'],
                                              tag_to_objects['target'])
        assert goal._last_step == 40


def test_ObjectPermanenceGoal_update_body():
    goal = ObjectPermanenceGoal()
    body = goal.update_body(copy.deepcopy(BODY_TEMPLATE), False)
    verify_body(body, goal)
    assert set(body['goal']['domain_list']) == set(
