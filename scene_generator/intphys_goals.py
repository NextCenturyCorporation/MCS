...
class IntPhysGoal(Goal, ABC):
        ...
        for _ in range(scenery_count):
            ...
            while location is None:
                ...
                location = geometry.calc_obj_pos(
                    geometry.ORIGIN, scenery_rects, scenery_def, random_x, random_z)
    ...
    def _get_occluders(self, obj_list: List[Dict[str, Any]],
        ...
        for i in range(num_paired_occluders):
            ...
            occluder_objs = self._get_paired_occluder(
                paired_obj, occluder_list, non_room_wall_materials, materials.METAL_MATERIALS)
    ...
    def _add_occluders(self, occluder_list: List[Dict[str, Any]],
        ...
        for _ in range(num_to_add):
            ...
            if occluder_fits:
                occluder_objs = objects.create_occluder(
                    random.choice(non_room_wall_materials), random.choice(
                        materials.METAL_MATERIALS), occluder_x, x_scale, sideways)
                occluder_list.extend(occluder_objs)
            else:
        ...
        for i in range(num_objects):
            ...
            if location in (
                ...
                max_x = IntPhysGoal.VIEWPORT_LIMIT_NEAR + \
                    obj_def['scale']['x'] / 2.0 * IntPhysGoal.VIEWPORT_PERSPECTIVE_FACTOR
            else:
    ...
    def _get_objects_falling_down(self, room_wall_material_name: str) \
        ...
        min_obj_distance = (IntPhysGoal.MIN_OCCLUDER_SCALE / 2 + IntPhysGoal.MIN_OCCLUDER_SCALE /
                            2 + MIN_OCCLUDER_SEPARATION) / IntPhysGoal.FAR_X_PERSPECTIVE_FACTOR
        ...
        for i in range(num_objects):
            ...
            for occluder in occluders:
                distance = abs(
                    ...
                    x_position)
                scale = 2 * \
                    (distance - occluder['shows'][0]['scale']['x'] / 2.0 - MIN_OCCLUDER_SEPARATION)
                if scale < 0:
                    raise exceptions.SceneException(
            ...
            occluder_pair = objects.create_occluder(
                random.choice(non_room_wall_materials), random.choice(
                    materials.METAL_MATERIALS), adjusted_x, x_scale, True)
...
class GravityGoal(IntPhysGoal):
    ...
    def __init__(self,
                 ramp_type: Optional[ramps.Ramp] = None,
                 roll_down: Optional[bool] = None,
    ...
    def is_ramp_steep(self) -> bool:
        ...
        return self._ramp_type in (
            ramps.Ramp.RAMP_90,
            ramps.Ramp.RAMP_30_90,
            ramps.Ramp.RAMP_45_90)
    ...
    def _get_ramp_and_objects(self, room_wall_material_name: str) -> \
        ...
        objs = self._get_objects_moving_across(
            ...
            self._last_step -
            IntPhysGoal.LAST_STEP_RAMP_BUFFER,
            valid_defs,
            0,
            valid_positions,
            positions)
        ...
        for i in range(len(objs)):
            ...
            if left_to_right and position in (
                    ...
                    IntPhysGoal.Position.LEFT_LAST_NEAR,
                    IntPhysGoal.Position.LEFT_FIRST_FAR,
                    IntPhysGoal.Position.LEFT_LAST_FAR) or not left_to_right and position in (
                    IntPhysGoal.Position.RIGHT_FIRST_NEAR,
                    IntPhysGoal.Position.RIGHT_LAST_NEAR,
                    IntPhysGoal.Position.RIGHT_FIRST_FAR,
                    IntPhysGoal.Position.RIGHT_LAST_FAR):
...
class SpatioTemporalContinuityGoal(IntPhysGoal):
    ...
    def _get_occluders(self, obj_list: List[Dict[str, Any]],
        ...
        for _ in range(2):
            occluder_objs = self._get_paired_occluder(
                target, occluder_list, non_room_wall_materials, materials.METAL_MATERIALS)
