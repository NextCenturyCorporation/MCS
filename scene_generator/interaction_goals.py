...
def parse_path_section(path_section: List[List[float]], current_heading: float, performer, goal_boundary) -> Tuple[List[Dict[str, Any]], float]:
    ...
    actions = []
    dx = path_section[1][0]-performer[0][0]
    dz = path_section[1][1]-performer[0][1]
...
class RetrievalGoal(InteractionGoal):
    ...
    def find_optimal_path(self, goal_objects: List[Dict[str, Any]], all_objects: List[Dict[str, Any]]) -> \
        ...
        plane_dist = math.sqrt( (goal[0]-performer[0] ) ** 2 + (goal[1]-performer[1]) ** 2 )
        height_dist = PERFORMER_CAMERA_Y-goal_objects[0]['shows'][0]['position']['y']
        elevation = degrees(math.atan2(height_dist,plane_dist))
        if abs(elevation) > 30:
            actions.append(
                 {
                'action': 'RotateLook',
