...
def verify_confusor(target, confusor):
    ...
    is_same_size = target['size'] == confusor['size'] and (
        target['dimensions']['x'] -
        util.MAX_SIZE_DIFFERENCE) <= confusor['dimensions']['x'] <= (
        target['dimensions']['x'] +
        util.MAX_SIZE_DIFFERENCE) and (
            target['dimensions']['y'] -
            util.MAX_SIZE_DIFFERENCE) <= confusor['dimensions']['y'] <= (
                target['dimensions']['y'] +
                util.MAX_SIZE_DIFFERENCE) and (
                    target['dimensions']['z'] -
                    util.MAX_SIZE_DIFFERENCE) <= confusor['dimensions']['z'] <= (
                        target['dimensions']['z'] +
...
def verify_obstructor(goal, scene, obstruct_vision=False):
    ...
    is_bigger = (
        target['dimensions']['x'] <= dimensions['x'] or target['dimensions']['z'] <= dimensions['z']) and (
...
def verify_pair(
        ...
        scene_1,
        scene_2,
        target_same_position=True,
        confusor_same_position=True,
        target_parent_same_position=True,
        confusor_parent_same_position=True,
        target_parent_same=True,
    ...
    if not verify_same_object('target', target_1, target_2, ([] if target_same_position else [
                              'shows']) + ([] if target_parent_same else ['locationParent'])):
        return False
    ...
    if confusor_1 and confusor_2:
        if not verify_same_object('confusor', confusor_1, confusor_2, ([] if confusor_same_position else [
                                  'shows']) + ([] if confusor_parent_same else ['locationParent'])):
            return False

    # If each scene has a confusor with a parent, ensure each confusor's
    # parent is the same.
    confusor_parent_1 = get_parent(
        confusor_1, scene_1['objects']) if (
        confusor_1 and 'locationParent' in confusor_1) else None
    print(f'[DEBUG] confusor_parent_1={confusor_parent_1}')
    confusor_parent_2 = get_parent(
        confusor_2, scene_2['objects']) if (
        confusor_2 and 'locationParent' in confusor_2) else None
    ...
    target_parent_id = target_parent_1['id'] if target_parent_1 else (
    ...
    confusor_parent_id = confusor_parent_1['id'] if confusor_parent_1 else (
...
def test_Pair4():
    ...
    if containerize:
        ...
        assert not geometry.are_adjacent(
            get_parent(
                target_2, scene_2['objects']), get_parent(
                confusor_2, scene_2['objects']))
    else:
        assert 'locationParent' not in target_1
        assert 'locationParent' not in target_2
        assert 'locationParent' not in confusor_2
    ...
    if containerize:
        ...
        assert verify_not_immediately_visible(
            ...
            get_parent(
                ...
                scene_2['objects']))
        assert verify_not_immediately_visible(
            pair._goal_1.get_performer_start(), scene_1['objects'], get_parent(
                confusor_1, scene_1['objects']), 'confusor')
        assert verify_immediately_visible(
            pair._goal_2.get_performer_start(),
            scene_2['objects'],
            get_parent(
