    ...
    expected_actions = [
        {
            'action': 'RotateLook', 'params': {
                'rotation': 315.0, 'horizon': 0.0}}, {
            'action': 'MoveAhead', 'params': {
                'amount': round(
                    math.sqrt(
                        2 * 0.1**2) / MAX_MOVE_DISTANCE, POSITION_DIGITS)}}]
    ...
    expected_actions = [
        {
            ...
            'action': 'MoveAhead', 'params': {
                'amount': round(
                    math.sqrt(
                        2 * 0.09**2) / MAX_MOVE_DISTANCE, POSITION_DIGITS)}}]
    start = {
        'position': {
            'x': 0,
            'y': 0,
...
def test_TransferToObjectRule_validate_location():
    ...
    assert rule.validate_location(
        location,
        [target],
        performer_start) == (
        geometry.position_distance(
            target_location['position'],
            location['position']) >= geometry.MIN_OBJECTS_SEPARATION_DISTANCE)
