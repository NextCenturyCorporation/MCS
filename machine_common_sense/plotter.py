...
class TopDownPlotter():
    ...
    def _initialize_plot(self, scene_config: SceneConfiguration) -> np.ndarray:
        ...
        img = self._draw_holes(img, scene_config.holes)
        img = self._draw_lava(img, scene_config.lava)
    ...
    def _draw_lava(self, img: np.ndarray, lava: List) -> np.ndarray:
        if lava is None:
            return img
        ...
        for area in lava:
            ...
            area_pos_ul = area_center - half_cell_pos
            area_pos_lr = area_center + half_cell_pos
