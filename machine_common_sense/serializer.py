...
class ISerializer:
    ...
    @classmethod
    def version(cls): return "0.1.0"

    @staticmethod
    @abstractmethod
    def serialize(unpacked_object): raise NotImplementedError

    @staticmethod
    @abstractmethod
    def deserialize(packed_object): raise NotImplementedError
...
class SerializerMsgPack(ISerializer):
    ...
    def _ext_pack(x):
        ...
        if isinstance(x, StepMetadata):
            return msgpack.ExtType(1, msgpack.packb([x.action_list, x.camera_aspect_ratio, x.camera_clipping_planes,
                                                     ...
                                                     x.structural_object_list], default=SerializerMsgPack._ext_pack,
                                                    strict_types=True))
        elif isinstance(x, tuple):
            return msgpack.ExtType(2, msgpack.packb([x[0], x[1]], default=SerializerMsgPack._ext_pack,
                                                    strict_types=True))
        elif isinstance(x, Image.Image):
            return msgpack.ExtType(3, msgpack.packb(SerializerMsgPack.image_to_bytes(x),
            ...
            return msgpack.ExtType(4, msgpack.packb([x.action_list, x.category, x.description, x.habituation_total,
                                                     x.last_preview_phase_step, x.last_step, x.metadata],
    ...
    def _ext_unpack(code, data):
        ...
        if code == 1:
            ...
            action_list, category, description, habituation_total, last_preview_phase_step, last_step, metadata = \
                msgpack.unpackb(data, ext_hook=SerializerMsgPack._ext_unpack)
            return GoalMetadata(action_list, category, description, habituation_total, last_preview_phase_step,
                                last_step, metadata)
        elif code == 5:
    ...
    def serialize(step_metadata: mcs.StepMetadata):
        ...
        You can use `object_to_persist = {'payload': step_metadata, 'additional_info': 'info'}` to add extra data.
